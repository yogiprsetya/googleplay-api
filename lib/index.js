...
const cleanFintechUrls = (req) => (app) => Object.assign({}, app, {
  playstoreUrl: app.url,
  url: buildUrl(req, 'fintech/' + app.appId),
});
...
/* Fintech detail */
router.get('/fintech/:appId', function (req, res, next) {
  const opts = Object.assign({ appId: req.params.appId }, req.query);
  gplay.fintech(opts)
    .then(cleanFintechUrls(req))
    .then(res.json.bind(res))
    .catch(next);
});
